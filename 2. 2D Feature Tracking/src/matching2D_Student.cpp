...
void detKeypointsHarris(std::vector<cv::KeyPoint> &keypoints, cv::Mat &img, bool bVis) 
{
    ...
    cv::convertScaleAbs(dst_norm, dst_norm_sc);
    ...
    for (int i=0; i < img.rows; ++i) 
    {
        for (int j=0; j < img.cols; ++j) 
        {
            ...
            {
                cv::KeyPoint test_keypoint;
                test_keypoint.pt = cv::Point2f(j, i);
                test_keypoint.size = kSize * 2;
                test_keypoint.response = response;

                bool overlap = false;
                for (auto it=keypoints.begin(); it != keypoints.end(); it++) 
                {
                  if (cv::KeyPoint::overlap(test_keypoint, *it) > 0) 
                  {
                    overlap = true;
                    if (test_keypoint.response > it->response) 
                    {
                      *it = test_keypoint;
                      break;
                    }
                ...
                if (!overlap) 
                    keypoints.push_back(test_keypoint);
            }
        }
    }
    t = ((double)cv::getTickCount() - t) / cv::getTickFrequency();
...
void detKeypointsModern(std::vector<cv::KeyPoint> &keypoints, cv::Mat &img, std::string detectorType, bool bVis) {
    ...
    {
        cv::FastFeatureDetector::DetectorType detector_type = cv::FastFeatureDetector::TYPE_7_12;
        cv::Ptr<cv::FeatureDetector> detector = cv::FastFeatureDetector::create(30, true, detector_type);
        double t = (double)cv::getTickCount();
